...
export function Modal({ open, onClose, variant = 'panel', side = 'right', title, children, className }: ModalProps) {
  ...
  return (
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label={title} className={cn('fixed z-[400] flex flex-col bg-[var(--surface)]', isPanel && ['top-0 bottom-0 w-[680px] max-w-full shadow-[-8px_0_32px_rgba(0,0,0,0.5)]', side === 'right' ? 'right-0' : 'left-0', side === 'left' && 'shadow-[8px_0_32px_rgba(0,0,0,0.5)', 'animate-[slide-in-right_200ms_ease-out]'], !isPanel && ['top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2', 'w-[560px] max-w-[95vw] max-h-[80vh]', 'rounded-2xl border border-[var(--border)] shadow-2xl', 'animate-[modal-in_200ms_ease-out]'], className)}>
        {children}
      </div>
    </>
  );
...
export function ModalHeader({ onClose, className, children, ...props }: { onClose: () => void; className?: string; children: ReactNode } & HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('flex items-center gap-3 px-5 py-4 border-b border-[var(--border)]', className)} {...props}>
      {children}
      <button onClick={onClose} className="ml-auto p-2 text-[var(--text-2)] hover:text-[var(--text)] hover:bg-[var(--surface-2)] rounded-lg transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#FF3B30]/50" aria-label="Fermer">
