  ...
  return (
    <ToastContext.Provider value={{ addToast, removeToast }}>
      ...
      <div className="fixed bottom-4 right-4 z-[500] flex flex-col-reverse gap-2 max-w-[400px]" aria-live="polite">
        {toasts.map(t => (
          <div key={t.id} className="flex items-start gap-3 bg-[var(--surface-2)] border border-[var(--border)] border-l-4 rounded-xl p-3 shadow-2xl animate-[slide-in-up_200ms_ease-out]" role="alert">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-[var(--text)] leading-tight">{t.title}</p>{t.description && <p className="text-xs text-[var(--text-2)] mt-0.5">{t.description}</p>}</div>
            <button onClick={() => removeToast(t.id)} className="flex-shrink-0 p-1 text-[var(--text-3)] hover:text-[var(--text)]"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>
          </div>
        ))}
      </div>
