...
export default async function SearchPage({ searchParams }: { searchParams: { q?: string } }) {
  ...
  const results = query ? liveMatches.flatMap(group => group.matches).filter(match => `${match.homeTeam.name} ${match.awayTeam.name} ${match.league.name}`.toLowerCase().includes(query.toLowerCase())) : []
